<style>
    ...
    .group-selector-modal {
        ...
        padding: 16px;
        z-index: 2000; /* sit above modal content */
        min-width: 300px;
        max-width: 500px;
        max-height: 70vh;
        overflow-y: auto;
    ...
    /* Orange highlight for preview changes (more pronounced orange) */
    .timetable-cell.preview-change {
        background-color: rgba(255,152,0,0.08) !important; /* soft orange tint */
        border: 2px solid #ff9800 !important; /* orange border */
        position: relative;
    }

    /* Inline preview lesson inserted into a cell */
...
<script>
    ...
    async function loadSelectedStudentsTimetables() {
        ...
        try {
            ...
            window.timetablesForComparison.forEach((result, idx) => {
                ...
                html += `<div class="student-timetable-card-responsive" data-student-id="${result.id}">
                    <div class="student-timetable-card-header">
                        <h6 style="margin: 0;">
                            <a href="/admin/schools/<?php echo e($school->id); ?>/timetables/<?php echo e($timetable->id); ?>/student/${result.id}" 
            ...
            const modalElement = document.getElementById('selectedStudentsTimetablesModal');
    ...
    function initializeCellInteractions() {
        ...
        document.querySelectorAll('.comparison-cell').forEach(cell => {
            ...
            cell.addEventListener('click', async function(e) {
                ...
                // Don't do anything if this is a preview-change cell (yellow/orange)
                if (this.classList.contains('preview-change')) {
                    return;
                }
            });
        });
    ...
    async function openCheckForCell(day, slot, studentId, el) {
        ...
        document.querySelectorAll('.group-selector-modal').forEach(el => el.remove());
        
        // Find modal-body to append popup inside modal
        const modalBody = document.querySelector('#selectedStudentsTimetablesModal .modal-body');
        if (!modalBody) {
        ...
        modal.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div class="group-selector-modal-title" style="margin: 0;">
                    Pamokos vykstančios mokykloje (Pamoka ${slot}, ${getDayName(day)})
                </div>
                <button class="btn btn-sm btn-close" onclick="this.closest('.group-selector-modal').remove(); event.stopPropagation();" title="Uždaryti"></button>
            </div>
        ...
        // Append inside modal instead of body
        modalBody.appendChild(modal);
        console.log('Modal created and appended inside modal-body');
        
        // Close on outside click - use event delegation on the modal-body
        const closeHandler = (e) => {
            if (!e.target.closest('.group-selector-modal') && !e.target.closest('.group-selector-item') && e.target.closest('#selectedStudentsTimetablesModal')) {
                modal.remove();
                modalBody.removeEventListener('click', closeHandler);
            }
        };
        
        setTimeout(() => {
            modalBody.addEventListener('click', closeHandler);
        }, 100);
    ...
    // Fetches all lessons for the group and inserts them into all matching cells
    async function addGroupToCell(groupId, groupName, subject, teacher, day, slot, studentId) {
        // Close modals
        document.querySelectorAll('.group-selector-modal').forEach(el => el.remove());
        ...
        // Clear all previous preview insertions before adding new ones
        ...
        try {
            ...
            lessons.forEach(lesson => {
                ...
                const cell = currentStudentCard.querySelector(`.comparison-cell[data-day="${lessonDay}"][data-slot="${lessonSlot}"]`);
                if (!cell) {
                ...
                const hasExistingLesson = cellContent && cellContent.textContent.trim() !== '' && 
                                        (cellContent.querySelector('small') !== null || 
                                         cellContent.textContent.includes('Laisva') === false);
                
                // Add preview div to cell
                if (cellContent) {
                    ...
                    `;
                    // Append preview after the main content div, directly to the <td>
