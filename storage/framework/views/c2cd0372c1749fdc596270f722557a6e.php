                                                ...
                                                <?php $__currentLoopData = $cell; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <?php
                                                        ...
                                                        $tooltipHtml = '<div class="tt-inner">'
                                                            ...
                                                            .'<div class="tt-row"><i class="bi bi-collection-fill tt-ico"></i><span class="tt-val">'.e($entry['group']).'</span></div>'
                                                            .'<div class="tt-row"><i class="bi bi-book-half tt-ico"></i><span class="tt-val">'.e($entry['subject'] ?? '—').'</span></div>'
    ...
    document.querySelectorAll(UNSCHEDULED_SELECTOR).forEach(el => {
        el.addEventListener('dragstart', e => {
            ...
            e.dataTransfer.setData('application/json', JSON.stringify({
                ...
                teacherId: el.dataset.teacherId,
                canMerge: el.dataset.canMerge === '1'
            }));
            ...
            if (draggedKind === 'unscheduled') {
                ...
                try {
                    const resp = await fetch(`<?php echo e(route('schools.timetables.manual-slot', [$school, $timetable])); ?>`, {
                        ...
                        body: JSON.stringify({ group_id: groupId, teacher_id: teacherId, day: day, slot: slot, merge: (meta.canMerge && !!cell.querySelector('.tt-trigger') && (subjectName === (cell.querySelector('.tt-trigger')?.dataset.subjectName || '')) && (cell.querySelector('.tt-trigger')?.dataset.canMerge === '1')) })
                    });
                    ...
                    const tooltipHtml = `<div class=\"tt-inner\">`
                      ...
                      + `</div>`;
                                        const b64 = btoa(unescape(encodeURIComponent(tooltipHtml)));
                                                                                const newBadgeHtml = `<span class=\"badge bg-secondary tt-trigger d-block mb-1\" style=\"font-size:0.75rem; cursor:move;\" data-tooltip-b64=\"${b64}\" draggable=\"true\"
                                                data-kind=\"scheduled\"
                                        ...
                                        if (cell.querySelector('.tt-trigger')) {
                    ...
                    if (window.bootstrap) {
                                            ...
                                            new bootstrap.Tooltip(newBadge, { title: decoded, html: true, sanitize: false, placement: 'top', trigger: 'hover focus', delay:{show:120, hide:60} });
                                            initBadgeDrag(newBadge);
                    }
